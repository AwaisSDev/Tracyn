...
// The login / sign-up / verify-code screen. Lives here (not in the page
// file) so it can take starting state: app/login/page.tsx renders it with
// none, and a local preview can open it straight on sign-up or on the
// verify step without submitting anything.
export function LoginView({
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (mode === "signup") {
        // Checked before anything is sent, so a typo never burns one of
        // the rate-limited verification emails below.
      ...
      >
        <form onSubmit={handleVerify} className="space-y-4">
          <input
            ...
            maxLength={10}
            // No spaces: the 0.4em tracking already spaces the dots, and the
            // matching left padding offsets the gap tracking adds after the
            // last character, so the code sits truly centered.
            placeholder="••••••••"
            aria-label="Verification code"
            value={code}
            onChange={(e) => setCode(e.target.value.replace(/\D/g, ""))}
            className="h-14 w-full rounded-[12px] border border-[#c9d6f7] bg-[#eef3ff] pl-[0.4em] text-center font-mono text-2xl font-semibold tracking-[0.4em] text-[var(--cv-blue-bright)] outline-none transition-[border-color,box-shadow] placeholder:text-[#9fb2ea] focus:border-[var(--cv-blue-bright)] focus:shadow-[0_0_0_4px_rgba(53,83,212,0.12)]"
          />
          {error &&
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {mode === "signup" && (
          <div className="space-y-1.5">
            <label htmlFor="confirm-password" className="text-[13.5px] font-medium text-[var(--cv-ink)]">
              Confirm password
            </label>
            <input
              id="confirm-password"
              ...
              className={cn(
                ...
                confirmPassword.length > 0 &&
                  ...
                  "border-[#e0a3a0] focus:border-[#d4483f] focus:shadow-[0_0_0_4px_rgba(212,72,63,0.12)]"
              )}
            />
          </div>
        )}
